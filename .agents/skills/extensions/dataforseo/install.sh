...
main() {
    ...
    if [ ! -d "${SEO_SKILL_DIR}" ]; then
        _GLOB_MATCH=$(ls -d "${HOME}/.claude/plugins/cache/seo-skills/"*/skills/seo 2>/dev/null | tail -n1 || true)
    ...
    if [ ! -d "${SEO_SKILL_DIR}" ]; then
        ...
        echo "  Install the seo-skills base plugin first."
        exit 1
    fi
    ...
    if [ -f "${SCRIPT_DIR}/skills/seo-dataforseo/SKILL.md" ]; then
        ...
        echo "  Run this script from the seo-skills repo: ./extensions/dataforseo/install.sh"
        exit 1
    fi
